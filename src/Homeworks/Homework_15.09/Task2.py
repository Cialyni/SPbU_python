...
def vectors_angle(vec1, vec2):
    return round(math.acos(scalar_multiplication(vec1, vec2)/ (vector_len(vec1) * vector_len(vec2))) * 180 / math.pi, 3)
...
def vector():
    ...
    while key != 0:
        ...
        if key == 3:
            vec1 = vec_input(1)
            vec2 = vec_input(2)
            print('\033[1m{}{}\033[0m'.format('\nScalar multiplication of this vector = ', scalar_multiplication(vec1, vec2)))
        if key == 1:
            vec = vec_input()
...
def matrix_addition(matr1, matr2):
    return matr1[0], matr1[1], [[matr1[2][i][j] + matr2[2][i][j] for j in range(matr1[1])] for i in range(matr1[0])]

def matrix_multiplication(matr1, matr2):
    return matr1[0], matr1[1], [[sum(matr1[2][i][k] * matr2[2][k][j] for k in range(matr1[1])) for j in range(matr2[1])] for i in range(matr1[0])]
...
def transponirovanie(matr):
    return matr[1], matr[0], [[matr[2][i][j] for i in range(matr[0])] for j in range(matr[1])]
